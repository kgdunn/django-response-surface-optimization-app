...
def show_one_system_other(request, short_name_slug, other_slug):
    """When a person wants to inspect the system of another"""
    other = models.Person.objects.filter(slug=other_slug)
    if len(other):
        return show_one_system(request, short_name_slug, other=other[0])
    else:
        return show_one_system(request, short_name_slug, force_GET=True,
                                                extend_dict={'message': ('')})
...
def show_one_system(request, short_name_slug, force_GET=False, extend_dict={},
    ...
    leads = fetch_leaderboard_results_one_system(system=system,
                                                 person=true_person)
...
def fetch_leaderboard_results_one_system(system=None, person=None):
    ...
    for idx, persyst in enumerate(persysts):
        ...
        leads.append([persyst.get_score(),
                      ...
                      persyst.person.slug,
                      you,
                      0])  # <-- placeholder: gets replaced just below
